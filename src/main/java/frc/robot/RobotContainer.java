...
public class RobotContainer {
  ...
  public static final CommandXboxController m_manipulatorController = new CommandXboxController(
  ...
  // MoveIntakeC m_MoveIntakeDownC = new MoveIntakeC(m_MoveIntakeS, .25);//mess
  // with p so it work(use the PID in HoodAngles) placeholding :)
  // MoveIntakeC m_MoveIntakeUpC = new MoveIntakeC(m_MoveIntakeS, 0);
  XLock m_XLock = new XLock(m_SwerveS);
  // ShooterHoodC m_ShooterHoodNegativeC = new ShooterHoodC(m_HoodAngleS,
  // Constants.HoodConstants.hoodAngleVoltageNegative);
  // ShooterHoodC m_ShooterHoodPositiveC = new ShooterHoodC(m_HoodAngleS,
  // Constants.HoodConstants.hoodAngleVoltagePositive);
  // ShooterHoodC m_ShooterHoodStopC = new ShooterHoodC(m_HoodAngleS, 0);

  // ShooterC m_ShooterC = new ShooterC(m_ShooterS,
  // Constants.ShooterConstants.shooter1Voltage,
  // Constants.ShooterConstants.shooter2voltage);
  // IntakeRollerC m_IntakeRollerC = new IntakeRollerC(m_IntakeRollerS,
  // Constants.IntakeRollerConstants.rollerVoltage);
  // IntakeRollerC m_IntakeRollerOffC = new IntakeRollerC(m_IntakeRollerS, 0);
  // HangC m_HangC = new HangC(m_HangS,
  // Constants.IntakeRollerConstants.rollerVoltage);
  // IndexerC m_IndexerC = new IndexerC(m_IndexerS,
  // Constants.IndexerConstants.indexerVoltage);
  // RollerC m_RollerStopC = new RollerC(m_IntakeRollerS, 0); should be
  // unnecessary bcz m_RollerC is on a toggleOnTrue

  // ParallelCommandGroup shootAndIndex = new ParallelCommandGroup(Commands.run(()
  // -> m_ShooterS.fire(Constants.ShooterConstants.shooter1Voltage,
  // Constants.ShooterConstants.shooter2voltage),m_ShooterS).finallyDo(() ->
  // m_ShooterS.fire(-2, -2)), Commands.run(() ->
  // m_IndexerS.setVoltage(Constants.IndexerConstants.indexerVoltage),
  // m_IndexerS).finallyDo(() -> m_IndexerS.setVoltage(0)));
  ...
  private void configureBindings() {
    ...
    // rDriverBumper.toggleOnTrue(Commands.run(() ->
    // m_HangS.hangPower(Constants.HangConstants.hangVoltage), m_HangS).finallyDo(()
    // -> m_HangS.hangPower(0)));
    aDriverButton.onTrue(new InstantCommand(() -> RobotContainer.linearSpeedMultiplier = 10)
        .finallyDo(() -> RobotContainer.linearSpeedMultiplier = 7.5));

    bManipulatorButton
        .whileTrue(new InstantCommand(() -> Robot.intakeSetpoint = Constants.IntakeConstants.downPositionSetpoint));
    yManipulatorButton
        .whileTrue(new InstantCommand(() -> Robot.intakeSetpoint = Constants.IntakeConstants.upPositionSetpoint));

    // bManipulatorButton.whileTrue((Commands.run(() ->
    // m_MoveIntakeS.moveTo(Constants.IntakeConstants.downPositionSetpoint),m_MoveIntakeS)).finallyDo(()
    // -> m_MoveIntakeS.setVoltage(0)));
    // yManipulatorButton.whileTrue((Commands.run(() ->
    // m_MoveIntakeS.moveTo(Constants.IntakeConstants.upPositionSetpoint),m_MoveIntakeS)).finallyDo(()
    // -> m_MoveIntakeS.setVoltage(0)));

    xManipulatorButton.whileTrue(
        Commands.run(() -> m_IntakeRollerS.rollerSpeed(Constants.IntakeRollerConstants.rollerVoltage), m_IntakeRollerS)
            .finallyDo(() -> m_IntakeRollerS.rollerSpeed(0)));
    dpadManipDown.whileTrue(Commands
        .run(() -> m_IntakeRollerS.rollerSpeed(-1 * Constants.IntakeRollerConstants.rollerVoltage), m_IntakeRollerS)
        .finallyDo(() -> m_IntakeRollerS.rollerSpeed(0)));
    ...
    // dpadManipLeft.onTrue(Commands.run(() -> m_HopperS.extendHopper(),
    // m_HopperS));
    // dpadManipRight.onTrue(Commands.run(() -> m_HopperS.retractHopper(),
    // m_HopperS));

    lManipulatorBumper.onTrue(new InstantCommand(() -> {
      hoodSetpoint = MathUtil.clamp(hoodSetpoint += 0.05, Constants.HoodConstants.minHoodRange,
          Constants.HoodConstants.maxHoodRange);
    ...
    // aManipulatorButton.whileTrue(Commands.run(() ->
    // m_ShooterS.fire(Constants.ShooterConstants.shooter1Voltage,
    // Constants.ShooterConstants.shooter2voltage),m_ShooterS).finallyDo(() ->
    // m_ShooterS.fire(0, 0)));
    // aManipulatorButton.whileTrue(Commands.run(() ->
    // m_IndexerS.setVoltage(Constants.IndexerConstants.indexerVoltage),
    // m_IndexerS).finallyDo(() -> m_IndexerS.setVoltage(0))); // to change it to
    // both on

    // aDriverButton.toggleOnTrue(new InstantCommand(() -> System.out.println("a
    // manip button")));
    // bDriverButton.toggleOnTrue(new InstantCommand(() -> System.out.println("b
    // manip button")));
    // xDriverButton.toggleOnTrue(new InstantCommand(() -> System.out.println("x
    // manip button")));
    // yDriverButton.toggleOnTrue(new InstantCommand(() -> System.out.println("y
    // manip button")));
    // rManipulatorBumper.or(lManipulatorBumper).whileFalse(m_ShooterHoodStopC);

    aManipulatorButton.whileTrue(Commands
        .run(() -> m_ShooterS.fire(Constants.ShooterConstants.shooter1Voltage,
            Constants.ShooterConstants.shooter2voltage), m_ShooterS)
        .finallyDo(() -> m_ShooterS.fire(0, -2))
        .raceWith(Commands.run(() -> m_IndexerS.setVoltage(Constants.IndexerConstants.indexerVoltage), m_IndexerS)
            .finallyDo(() -> m_IndexerS.setVoltage(0))));
